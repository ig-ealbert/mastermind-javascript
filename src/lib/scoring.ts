...
export function calculateBlackPegs(currentGuess: string[], answer: string[]) {
  return currentGuess.filter(
    (color: string, index: number) => color === answer[index]
  ).length;
}
...
export function calculateRemainingColors(
  ...
  const remainingColors = answerColors.slice();
  for (let i = 0; i < 4; i++) {
...
export function calculateWhitePegs(
  ...
  for (let i = 0; i < 4; i++) {
    if (
      answer.indexOf(currentGuess[i]) !== -1 &&
